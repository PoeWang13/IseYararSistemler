...
public class Leg : MonoBehaviour
{
    ...
    public float magenta = 0.1f;
    public float blue = 0.1f;
    public float yellow = 0.1f;
    private void OnDrawGizmos()
    {
        Gizmos.color = Color.magenta;
        Gizmos.DrawSphere(RaycastTipPos, magenta);

        Gizmos.color = Color.blue;
        Gizmos.DrawSphere(TipPos, blue);

        Gizmos.color = Color.red;
        Gizmos.DrawLine(TipPos, RaycastTipPos);

        Gizmos.color = Color.yellow;
        Gizmos.DrawSphere(ikTarget.transform.position, yellow);
    }
}